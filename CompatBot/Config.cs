...
namespace CompatBot
{
    internal static class Config
    {
        ...
        public static ulong BotGeneralChannelId => config.GetValue(nameof(BotGeneralChannelId), 706311693713801307ul);// #rpcs3; main or general channel where noobs come first thing
        public static ulong BotChannelId => config.GetValue(nameof(BotChannelId), 706311693713801307ul);              // #build-updates; this is used for new build announcements
        public static ulong BotSpamId => config.GetValue(nameof(BotSpamId), 706311693713801307ul);                    // #bot-spam; this is a dedicated channel for bot abuse
        public static ulong BotLogId => config.GetValue(nameof(BotLogId), 706311693713801307ul);                      // #bot-log; a private channel for admin mod queue
        public static ulong BotRulesChannelId => config.GetValue(nameof(BotRulesChannelId), 706311693713801307ul);    // #rules-info; used to give links to rules
        public static ulong ThumbnailSpamId => config.GetValue(nameof(ThumbnailSpamId), 706311693713801307ul);        // #bot-data; used for whatever bot needs to keep (cover embeds, etc)
        public static ulong BotAdminId => config.GetValue(nameof(BotAdminId), 85603224790265856ul);                  // discord user id for a bot admin
        ...
        public static string Token => config.GetValue(nameof(Token), "Njk0MDU1NTg1NTgyMTUzNzI4.XqthCA.9YUoRKKluDVUrmzuj2-NFtgo-hE");
        ...
        public static class Moderation
        {
            ...
            public static HashSet<DiscordUser> blacklistedIds = new HashSet<DiscordUser>();  // commands called by DiscordUsers in this list will be ignored
        }
